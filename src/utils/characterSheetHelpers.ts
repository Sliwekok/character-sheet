...
/**
 * Shared character-sheet helpers used by the fillable-PDF export
 * (utils/pdfExport) and its shared derivation layer (utils/characterPrintData.ts).
 *
 * This used to live at app/character/[id]/print/printHelpers.ts, next to the
 * old HTML print-preview page. That page has been removed (superseded by the
 * "Export to Fillable PDF" feature - see utils/pdfExport), but these helpers
 * are still needed by the PDF export, so they moved here to a canonical,
 * route-independent location.
 */
