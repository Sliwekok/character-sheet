...
export const Ranger: CharacterClass = {
    ...
    subclassLevel: 3,
    // features text pulled and verified against 5etools' class-ranger.json (PHB source) - https://5e.tools/classes.html#ranger_phb
    // Corrected from an earlier from-memory pass: the two separate 6th-level "Favored Enemy
    // Improvement" and "Natural Explorer Improvement" entries were actually a single combined
    // feature ("Favored Enemy and Natural Explorer Improvements") on the class table, so they've
    // been merged; and a missing 10th-level "Natural Explorer Improvement" entry has been added
    // (previously only Hide in Plain Sight was listed at level 10, even though the table also
    // grants a third favored terrain there).
    features: [
