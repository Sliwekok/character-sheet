...
export const Fighter: CharacterClass = {
    ...
    subclassLevel: 3,
    // features text verified against 5etools' class-fighter.json (PHB source) - https://5e.tools/classes.html#fighter_phb
    // Corrected from an earlier from-memory pass: the level-2 and level-9 entries were named
    // "Action Surge (one use)" and "Indomitable (one use)", but the PHB only appends a
    // use-count suffix to the LATER, increased-use instances ("Action Surge (two uses)" at 17,
    // "Indomitable (two uses)" at 13 / "(three uses)" at 17) - the first instance of each is
    ...
    features: [
        ...
        { name: "Indomitable", level: 9, description: "You can reroll a saving throw that you fail. If you do so, you must use the new roll, and you can't use this feature again until you finish a long rest. You can use it twice between long rests starting at 13th level and three times starting at 17th level." },
